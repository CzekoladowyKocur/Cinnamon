...
namespace Cinnamon {
	...
	)
	{
		...
		if(pointerEvent->eventMask & POINTER_EVENT_MOTION)
		{
			...
			MouseMovedEvent event(static_cast<uint32_t>(wl_fixed_to_int(pointerEvent->x)), static_cast<uint32_t>(wl_fixed_to_int(pointerEvent->y)));
			window->SendEvent(event);
		}

		if(pointerEvent->eventMask & POINTER_EVENT_BUTTON)
		{
			if(pointerEvent->state == WL_POINTER_BUTTON_STATE_PRESSED)
			{
				MousePressedEvent event(pointerEvent->button);
				window->SendEvent(event);
			}
			else
			{
				MouseReleasedEvent event(pointerEvent->button);
				window->SendEvent(event);
			}
