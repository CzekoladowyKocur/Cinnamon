...
namespace Cinnamon {
	...
	uint32_t Input::GetMousePositionX()
	{
		const Application* application{ Application::Get() };
		return application->GetMainWindow()->GetInputState()->GetMousePosition().first;
	}

	uint32_t Input::GetMousePositionY()
	{
		const Application* application{ Application::Get() };
		return application->GetMainWindow()->GetInputState()->GetMousePosition().second;
	}
}